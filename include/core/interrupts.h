 ...
 *
 * Used to allow InterruptHandler to reference InterruptManager without including its full
 ...
 *
 * Provides a common interface for handling interrupts and manages the association with an
 * InterruptManager.
 */
class InterruptHandler {
protected:
 ...
 *
 * Sets up and manages the Interrupt Descriptor Table (IDT), Programmable Interrupt Controller
 * (PIC), and associated interrupt handlers.
 */
class InterruptManager {
    friend class InterruptHandler;  ///< Allows InterruptHandler to access private/protected members
                                    ///< of InterruptManager.
public:
...
protected:
    ...
    struct GateDescriptor {
        ...
        uint16_t gdt_codeSegmentSelector;  ///< Code segment selector in the Global Descriptor Table
                                           ///< (GDT).
...
public:
     ...
     *
     * @param gdt, processManager, vbe Pointers to GlobalDescriptorTable, ProcessManager and
     * GraphicsDriver
     */
